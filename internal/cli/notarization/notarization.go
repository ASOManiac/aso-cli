...
func notarizationCommand() *ffcli.Command {
	return &ffcli.Command{
		Name:       "notarization",
		ShortUsage: "aso notarization <subcommand> [flags]",
		ShortHelp:  "Manage macOS notarization submissions.",
		LongHelp: `Manage macOS notarization submissions via the Apple Notary API.

Examples:
  aso notarization submit --file ./MyApp.zip
  aso notarization submit --file ./MyApp.zip --wait
  aso notarization status --id "SUBMISSION_ID"
  aso notarization log --id "SUBMISSION_ID"
  aso notarization list`,
...
func submitCommand() *ffcli.Command {
	...
	return &ffcli.Command{
		Name:       "submit",
		ShortUsage: "aso notarization submit --file <path> [flags]",
...
Examples:
  aso notarization submit --file ./MyApp.zip
  aso notarization submit --file ./MyApp.zip --wait
  aso notarization submit --file ./MyApp.zip --wait --poll-interval 30s --timeout 1h
  aso notarization submit --file ./MyApp.zip --output table`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			...
			if !*wait {
				if shared.ProgressEnabled() {
					fmt.Fprintf(os.Stderr, "Use 'aso notarization status --id %s' to check progress.\n", submissionID)
				}
			...
			case asc.NotaryStatusInvalid, asc.NotaryStatusRejected:
				if shared.ProgressEnabled() {
					...
					fmt.Fprintf(os.Stderr, "Run 'aso notarization log --id %s' for details.\n", submissionID)
				}
...
func statusCommand() *ffcli.Command {
	...
	return &ffcli.Command{
		Name:       "status",
		ShortUsage: "aso notarization status --id \"SUBMISSION_ID\"",
...
Examples:
  aso notarization status --id "SUBMISSION_ID"
  aso notarization status --id "SUBMISSION_ID" --output table`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
...
func logCommand() *ffcli.Command {
	...
	return &ffcli.Command{
		Name:       "log",
		ShortUsage: "aso notarization log --id \"SUBMISSION_ID\"",
...
Examples:
  aso notarization log --id "SUBMISSION_ID"
  aso notarization log --id "SUBMISSION_ID" --output table`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			idValue := strings.TrimSpace(*submissionID)
...
func listCommand() *ffcli.Command {
	...
	return &ffcli.Command{
		Name:       "list",
		ShortUsage: "aso notarization list [flags]",
		ShortHelp:  "List previous notarization submissions.",
		LongHelp: `List previous notarization submissions.

Examples:
  aso notarization list
  aso notarization list --limit 5
  aso notarization list --output table`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			if *limit < 0 {
