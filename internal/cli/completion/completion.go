...
func bashScript(subcommands []string) string {
	...
	return fmt.Sprintf(`# bash completion for aso
_aso_completions() {
  local cur
  COMPREPLY=()
...
complete -F _aso_completions aso
`, words)
}
...
func zshScript(subcommands []string) string {
	...
	words := strings.Join(subcommands, " ")
	return fmt.Sprintf(`#compdef aso
...
func fishScript(subcommands []string) string {
	words := strings.Join(subcommands, " ")
	return fmt.Sprintf(`# fish completion for aso
complete -c aso -f -a '%s'
`, words)
}
