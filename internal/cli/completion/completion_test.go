...
func TestCompletionCommandValidationAndOutput(t *testing.T) {
	...
	if !strings.Contains(stdout, "complete -F _aso_completions aso") {
...
func TestCompletionScriptHelpers(t *testing.T) {
	...
	if !strings.Contains(zshScript([]string{"apps"}), "#compdef aso") {
		t.Fatalf("zsh script missing compdef header")
	}
	if !strings.Contains(fishScript([]string{"apps"}), "complete -c aso") {
