...
func RankCommand() *ffcli.Command {
	fs := flag.NewFlagSet("aso maniac rank", flag.ExitOnError)
	return &ffcli.Command{
		Name:       "rank",
		ShortUsage: "aso maniac rank <subcommand> [flags]",
...
func rankTrackCommand() *ffcli.Command {
	fs := flag.NewFlagSet("aso maniac rank track", flag.ExitOnError)
	...
	return &ffcli.Command{
		Name:       "track",
		ShortUsage: "aso maniac rank track <appId> --keywords camera,photo [flags]",
		ShortHelp:  "Add an app to rank tracking with specific keywords.",
		LongHelp: `Start tracking an app's keyword rankings.

Examples:
  aso maniac rank track 123456789 --keywords camera,photo
  aso maniac rank track 123456789 --keywords vpn --storefront GB`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			if len(args) == 0 {
...
func rankHistoryCommand() *ffcli.Command {
	fs := flag.NewFlagSet("aso maniac rank history", flag.ExitOnError)
	...
	return &ffcli.Command{
		Name:       "history",
		ShortUsage: "aso maniac rank history <appId> --keyword camera [flags]",
		ShortHelp:  "View historical rank data for an app's keyword.",
		LongHelp: `View rank history for a tracked app and keyword.

Examples:
  aso maniac rank history 123456789 --keyword camera
  aso maniac rank history 123456789 --keyword vpn --from 2026-01-01 --to 2026-03-01`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			if len(args) == 0 {
