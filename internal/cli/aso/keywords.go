...
func keywordsAnalyzeCommand() *ffcli.Command {
	fs := flag.NewFlagSet("aso aso keywords analyze", flag.ExitOnError)
	...
	return &ffcli.Command{
		Name:       "analyze",
		ShortUsage: "aso aso keywords analyze <keyword> [<keyword>...] [flags]",
...
Examples:
  aso aso keywords analyze "photo editor"
  aso aso keywords analyze camera photo --storefront GB
  aso aso keywords analyze vpn --fields popularity,difficulty`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			if len(args) == 0 {
...
func keywordsRecommendCommand() *ffcli.Command {
	fs := flag.NewFlagSet("aso aso keywords recommend", flag.ExitOnError)
	storefront := fs.String("storefront", "US", "App Store storefront code")
	limit := fs.Int("limit", 50, "Maximum number of recommendations")

	return &ffcli.Command{
		Name:       "recommend",
		ShortUsage: "aso aso keywords recommend <seed> [flags]",
...
Examples:
  aso aso keywords recommend "photo editor"
  aso aso keywords recommend camera --storefront GB --limit 25`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			if len(args) == 0 {
...
func keywordsBatchCommand() *ffcli.Command {
	fs := flag.NewFlagSet("aso aso keywords batch", flag.ExitOnError)
	storefronts := fs.String("storefronts", "US", "Comma-separated storefront codes")

	return &ffcli.Command{
		Name:       "batch",
		ShortUsage: "aso aso keywords batch <keyword> [<keyword>...] --storefronts US,GB,DE",
...
Examples:
  aso aso keywords batch camera photo vpn
  aso aso keywords batch "photo editor" "video editor" --storefronts US,GB,DE`,
		FlagSet:   fs,
		UsageFunc: shared.DefaultUsageFunc,
		Exec: func(ctx context.Context, args []string) error {
			if len(args) == 0 {
