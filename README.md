...
All original App Store Connect commands work out of the box. The `aso aso` command group adds AI-powered keyword research, competitor analysis, rank tracking, and more.
...
# Try a command
aso aso keywords suggest --app-id 1234567890 --storefront us
```
...
All premium commands live under `aso aso`. Authenticate once with `aso aso login`.

| Command | Description |
|---------|-------------|
| `aso aso login` | Authenticate with your ASO Maniac account via browser |
| `aso aso logout` | Remove stored ASO Maniac credentials |
| `aso aso whoami` | Display current user, plan, and API usage |
| `aso aso keywords` | Analyze, suggest, and batch-process keywords |
| `aso aso competitors` | Analyze competitor keyword overlap |
| `aso aso rank` | Track and view keyword ranking history |
| `aso aso trends` | View keyword popularity trends over time |
| `aso aso export` | Export keyword data in CSV, JSON, or TSV format |
| `aso aso dashboard` | View your ASO portfolio dashboard overview |
| `aso aso storefronts` | List all supported App Store storefronts |
...
aso aso keywords suggest --app-id 1234567890 --storefront us | jq '.keywords[:5]'

# Export to CSV for spreadsheets
aso aso export --app-id 1234567890 --format csv > keywords.csv
```
